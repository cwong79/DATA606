...
-   Take a random sample of size 50 from `price`. Using this sample, what is 
    ...
    My best point estimate of the population mean is $178,766.1
    
```{r}
samp_price <- sample(price, 50)
samp_mean <- mean(samp_price)
samp_mean
```
...
-   Since you have access to the population, simulate the sampling 
    ...
    Based on this sampling distribution, my guess for mean home price would be somewhere right below $180,000. The      graph seems to peak at this value.
    
    The population mean for this sampling distribution is $180,922.4.
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    Based on this sampling distribution, my guess for mean home price would be somewhere right above $180,000. The      graph seems to peak at this value.
    
    The population mean for this sampling distribution is $180,736.6.
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    In this instance, the spread for sample size of 150 is larger. I would choose the smaller sample size of 50 for     making estimates due to the smaller range.
