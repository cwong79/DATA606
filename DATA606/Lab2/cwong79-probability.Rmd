...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    The distribution of streak length is right skewed. Half of the shots result in 0 streaks.
    The typical streak length for this simulation is 0. The longest player's streak is 5.
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    Running the simulation again produced a similar looking bar plot. I deducted that this would likely happen      because the mathematical probability of making a shot lowers significantly with each attempt. The may be        some variance in data, however, it more than likely fall along the standard distribution.
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    It seems that the simulated shooter is actually doing better in this instance. The mean value of the            simulated shooter is closer to one. Also, the simulated shooter has more instances of streaks, I am             considering streaks with more than 1 consecutive shots. Based on this comparison, there is no evidence that     the hot hand model fits Kobe's shooting patterns.
