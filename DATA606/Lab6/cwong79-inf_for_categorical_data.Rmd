...
We must assume that the observations are independent. There is an overlap in the intervals for Spain in 2005 and 2012, however there is not enough evidence to say there is a difference in the athesim index for 2005 compared to 2012.

```{r}
# for 2005
sp05 <- subset(atheism, nationality == "Spain" & year == "2005")
sp05$nationality <- as.factor(as.character(sp05$nationality))
table(sp05$nationality, sp05$response)

# for 2012
sp12 <- subset(atheism, nationality == "Spain" & year == "2012")
sp12$nationality <- as.factor(as.character(sp12$nationality))
table(sp12$nationality, sp12$response)
...
For the United States there is not convincing evidence as the confidence intervals do not overlap.
...
Based on 57 countries, we will see close to 3 Type 1 errors at a significance level of 0.05.
...
I would apply into the formula and receive 9604 as a sample size.
